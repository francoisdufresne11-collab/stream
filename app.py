...
def index():
    ...
    for r in sorted(rl,key=lambda x:-x['created']):
        ...
        cards+=('<a href="/watch/'+rid+'" class="room-card">'
               ...
               '<span class="tag tag-v">&#128065; '+str(vc)+'</span></div>'
...
def watch_page(rid):
    ...
    j='<script>var ROOM_ID="'+rid+'";var ROOM_TITLE="'+safe+'";'+WA_JS+'</script>'
...
@app.route('/api/rooms')
def api_rooms():
    with _lock:return jsonify([{'id':r['id'],'title':r['title'],'viewers':len(r['viewers'])} for r in _rooms.values()])
...
'<button class="btn btn-secondary btn-sm" style="width:100%;margin-top:6px" id="btn-refresh">&#128260; Rafraichir les appareils</button>'
'</div>'
...
'<div style="font-size:.76em;color:var(--mu);margin-top:6px;padding:8px;background:var(--bi);border-radius:6px">'
'&#128161; Elgato, AVerMedia, HDMI capture... apparaissent comme cameras dans le navigateur.'
'</div>'
'<button class="btn btn-secondary btn-sm" style="width:100%;margin-top:6px" id="btn-refresh2">&#128260; Rafraichir</button>'
'</div>'
...
'<div id="bc-chat" class="chat-msgs" style="max-height:160px"></div>'
