...
HLS_URL="https://cdn.jsdelivr.net/npm/hls.js@1.5.13"
...
def page(title,body,extra_js="",extra_head=""):
    return (
        ...
        '<script src="'+SIO_URL+'"></script>'
        '<script src="'+HLS_URL+'"></script>'
        +extra_head+
        '</head><body>'
        +navbar()+
...
def index():
    ...
    for s in active_streams.values():
        sid2=s["id"]
        cards+=(
            '<a href="/watch/'+sid2+'" class="card" data-id="'+sid2+'">'
            '<div class="thumb"><span class="lb">LIVE</span>'
            '<div class="tov"><span class="pi">&#9654;</span></div></div>'
            '<div class="cinfo"><h3>'+s["title"]+'</h3>'
            '<span id="vc-'+sid2+'">'+str(s["viewers"])+' viewers</span>'
            '</div></a>'
        )
    empty_display="" if active_streams else ""
    body=(
        '<div class="home">'
        '<h1>&#127916; Streams en direct</h1>'
        '<div id="grid" class="grid">'
        '<div id="empty" class="empty-home"'+('' if not active_streams else ' style="display:none"')+'>'
    ...
    js=(
        ...
        '    var cs=g.querySelectorAll(".card");'
        '    for(var i=0;i<cs.length;i++){if(!ids[cs[i].dataset.id])cs[i].remove();}'
        '    st.forEach(function(s){'
        '      var c=g.querySelector(\'[data-id="\'+s.id+\'"]\');'
        '      if(!c){'
        '        c=document.createElement("a");c.href="/watch/"+s.id;'
        '        c.className="card";c.dataset.id=s.id;'
        '        c.innerHTML=\'<div class="thumb"><span class="lb">LIVE</span>'
        '<div class="tov"><span class="pi">&#9654;</span></div></div>'
        '<div class="cinfo"><h3>\'+s.title+\'</h3>'
        '<span id="vc-\'+s.id+\'">\'+s.viewers+\' viewers</span></div>\';'
        '        g.appendChild(c);'
        '      }else{'
        '        var v=document.getElementById("vc-"+s.id);'
        '        if(v)v.textContent=s.viewers+" viewers";'
        '      }'
        '    });'
        '  }).catch(function(){});'
        '}'
        'load();setInterval(load,5000);'
        'socket.on("stream_created",load);'
        'socket.on("stream_ended",load);'
        '</script>'
    )
...
def broadcast_page():
    body=(
        ...
        '<div class="bc-grid">'
        '<div class="icard">'
        '<h3>Diffuser depuis votre PC</h3>'
        ...
        '<p style="color:#a78bfa;font-weight:700;margin-bottom:8px">Instructions :</p>'
        '<ol class="how">'
    ...
    js=(
        ...
        '  var u=document.getElementById("surl").textContent;'
        ...
        '    l.innerHTML=st.map(function(s){'
        '      return \'<div class="asi">'
        '<a href="/watch/\'+s.id+\'">\'+s.title+\'</a>'
        '<span style="margin-left:auto">\'+s.viewers+\'v</span></div>\';'
        '    }).join("");'
        '  });'
...
def watch_page(sid):
    ...
    body=WATCH_BODY
    js='<script>var __SID="'+sid+'";var __TITLE="'+safe+'";'+WATCH_SCRIPT+'</script>'
...
def admin():
    ...
    for s in active_streams.values():
        ...
        rows+=(
            "<tr>"
            "<td><code>"+sid2+"</code></td>"
            "<td>"+s["title"]+"</td>"
            "<td>"+str(s["viewers"])+"</td>"
            "<td>"+_ts(s["created"])+"</td>"
            "<td>"
            '<a href="/watch/'+sid2+'" class="bsm bb" target="_blank">Voir</a> '
            '<form method="POST" action="/admin/kick/'+sid2+'" style="display:inline"'
            ' onsubmit="return confirm(&quot;Arreter ce stream ?&quot;)">'
            '<button type="submit" class="bsm br">Stop</button></form>'
            "</td></tr>"
        )
    tbl=(
        '<table class="adm-tbl">'
        '<thead><tr><th>ID</th><th>Titre</th><th>Viewers</th><th>Debut</th><th>Actions</th></tr></thead>'
        '<tbody>'+rows+'</tbody></table>'
    ) if rows else '<div class="empty">Aucun stream actif</div>'
    tv=sum(len(v) for v in stream_viewers.values())
    body=(
        '<div class="adm-wrap">'
        '<div class="adm-top">'
        '<h1>Dashboard Admin</h1>'
        '<a href="/admin/logout" class="btn btn-d">Deconnexion</a>'
        '</div>'
        ...
        '<div class="ci"><span class="ck">Python</span><code>3.11-slim Docker</code></div>'
        '<div class="ci"><span class="ck">Worker</span><code>gthread -w1 -t100</code></div>'
        '<div class="ci"><span class="ck">FullScreen</span><code>F / double-clic</code></div>'
        '<div class="ci"><span class="ck">Son</span><code>Bouton violet si bloque</code></div>'
        '</div></div>'
        '<p class="adm-ref">Actualisation toutes les 10s</p>'
        '</div>'
    )
    head='<meta http-equiv="refresh" content="10">'
    return page("Dashboard Admin",body,extra_head=head)
...
def admin_kick(sid):
    ...
    _cleanup(sid)
    socketio.emit("stream_ended",{"stream_id":sid})
    return redirect(url_for("admin"))
...
def api_info(sid):
    if sid not in active_streams:return jsonify({"error":"inconnu"}),404
    m=dict(active_streams[sid])
    m["chat_count"]=len(chat_history.get(sid,[]))
    return jsonify(m)
...
def _leave(data):
    ...
    with _lock:
        if sid in stream_viewers:
            stream_viewers[sid].discard(request.sid)
            cnt=len(stream_viewers[sid])
...
'var ovt2=document.getElementById("ovt2");'
...
'  mup.setAttribute("d",IVO);vol.value=1;'
...
'    hls=new Hls({'
'      lowLatencyMode:true,liveSyncDurationCount:1,liveMaxLatencyDurationCount:3,'
'      maxLiveSyncPlaybackRate:1.5,maxBufferLength:4,maxMaxBufferLength:8,'
'      backBufferLength:4,startLevel:-1,'
'      manifestLoadingMaxRetry:20,manifestLoadingRetryDelay:500,'
'      fragLoadingMaxRetry:20,fragLoadingRetryDelay:500'
'    });'
...
'        o.value=i;o.textContent=lv.height+"p";'
'        qsel.appendChild(o);'
'      });'
'      retryCount=0;'
'      vid.muted=false;vid.volume=1;'
...
'        mup.setAttribute("d",IMU);vol.value=0;'
'      });'
'    });'
...
'        lat.textContent=delay>0?Math.round(delay)+"s":"";'
'      }'
'    });'
...
'};'
'function tFS(){'
'  if(!document.fullscreenElement&&!document.webkitFullscreenElement)eFS();else xFS();'
'}'
'function eFS(){'
'  var fn=pfsw.requestFullscreen||pfsw.webkitRequestFullscreen||pfsw.mozRequestFullScreen||pfsw.msRequestFullscreen;'
'  if(fn)fn.call(pfsw);'
'}'
'function xFS(){'
...
'bfs.onclick=tFS;'
'function sH(){'
'  cH();cbar.classList.remove("ch");document.body.style.cursor="";'
...
'  chatm.appendChild(d);'
'  while(chatm.children.length>200)chatm.removeChild(chatm.firstChild);'
'}'
'function esc(s){var d=document.createElement("div");d.textContent=s;return d.innerHTML;}'
'function sM(){'
'  var t=ci.value.trim();if(!t)return;'
'  var n=uname.value.trim()||"Anonyme";'
'  localStorage.setItem("sc_un",n);uname.value=n;'
'  socket.emit("chat_msg",{stream_id:SID,username:n,text:t});'
'  ci.value="";'
'}'
'cs.onclick=sM;'
'ci.addEventListener("keypress",function(e){if(e.key==="Enter")sM();});'
'function pN(u,t){'
...
# Note: __SID__ et __TITLE__ sont remplaces par de vraies valeurs JS
# On utilise __SID__ => "valeur" et __TITLE__ => "valeur"
WATCH_SCRIPT2=WATCH_SCRIPT.replace('var SID=__SID__;','').replace('var TITLE=__TITLE__;','')
